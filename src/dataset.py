...
def load_single_video_data(video_name, feature_dir, label_dir, event_list, sample_rate=1, temporal_aug=True, boundary_smooth=3):
    """
    加载单个视频的特征和标签数据
    ...
    Args:
        ...
        event_list: 事件列表
        sample_rate: 采样率
        temporal_aug: 是否进行时间增强
        boundary_smooth: 边界平滑参数

    Returns:
        data_dict: 包含单个视频数据的字典
        frame_rate: 帧率（帧/秒）
    """
    ...
    feature_file = os.path.join(feature_dir, '{}.npy'.format(video_name[:-4]))
    event_file = os.path.join(label_dir, video_name)

    if not os.path.exists(feature_file):
        raise FileNotFoundError(f"Feature file not found: {feature_file}")
    if not os.path.exists(event_file):
        raise FileNotFoundError(f"Label file not found: {event_file}")
    ...
    # 计算帧率（假设标签文件每帧对应1秒）
    # 如果标签文件是按帧标注的，需要根据实际情况调整
    frame_rate = 1.0  # 默认每秒1帧

    event_seq_raw = np.zeros(min(np.shape(feature)[1], frame_num))
    for i in range(len(event_seq_raw)):
    ...
    assert (feature.shape[1] == event_seq_raw.shape[0]), 'must match {} and {}'.format(feature.shape[1],
                                                                                       event_seq_raw.shape[0])
    ...
    data_dict[video_name]['feature'] = [torch.from_numpy(i).float() for i in feature]
    data_dict[video_name]['event_seq_raw'] = torch.from_numpy(event_seq_raw).float()
    data_dict[video_name]['event_seq_ext'] = [torch.from_numpy(i).float() for i in event_seq_ext]
    data_dict[video_name]['boundary_seq_raw'] = torch.from_numpy(boundary_seq_raw).float()
    data_dict[video_name]['boundary_seq_ext'] = [torch.from_numpy(i).float() for i in boundary_seq_ext]

    return data_dict, frame_rate
...
def get_data_dict(feature_dir, label_dir, video_list, event_list, sample_rate=4, temporal_aug=True, boundary_smooth=None):
    ...
    for video in tqdm(video_list):
        feature_file = os.path.join(feature_dir, '{}.npy'.format(video[:-4]))
        event_file = os.path.join(label_dir, video)
        feature = np.load(feature_file, allow_pickle=True)
        event = np.loadtxt(event_file, dtype=str)
        frame_num = len(event)
...
def get_boundary_seq(event_seq, boundary_smooth=None):
    ...
    _, start_times, end_times = get_labels_start_end_time([int(i) for i in event_seq])
    boundaries = start_times[0:]
